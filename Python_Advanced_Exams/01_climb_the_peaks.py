...
He will take his video set, a tent, and his backpack to the mountain.
The backpack fits food portions for one week, exactly.
...
daily_food = [int(i) for i in input().split(", ")]
daily_stamina = deque([int(i) for i in input().split(", ")])
peaks = deque(
    [("Vihren", 80),
     ("Kutelo", 90),
     ("Banski Suhodol", 100),
...
climbed_peaks = []
while daily_food and daily_stamina and peaks:
    food = daily_food.pop()
    stamina = daily_stamina.popleft()
    resource = food + stamina
